...
pub struct ZoneData {
    ...
    /// The planet index this zone belongs to (1 = Junon, 2 = Luna, 3 = Eldeon, etc.)
    pub planet: usize,
}
...
impl ZoneDatabase {
    ...
    /// Returns an iterator over all zone IDs in the database
    pub fn iter_zone_ids(&self) -> impl Iterator<Item = ZoneId> + '_ {
        self.zones
            .iter()
            .filter_map(|zone_data| zone_data.as_ref().map(|z| z.id))
    }
