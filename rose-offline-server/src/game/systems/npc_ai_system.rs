...
fn ai_action_message(
    ...
    let opt_npc_name = ai_system_resources
        .game_data
        .npcs
    ...
    if let Some(message) = ai_system_resources.game_data.ai.get_ai_string(string_id) {
        match message_type {
            ...
            AipMessageType::Shout => {
                if let Some(name) = opt_npc_name.as_ref() {
                    ai_system_parameters.server_messages.send_entity_message(
                        ai_parameters.source.client_entity,
...
pub fn npc_ai_system(
    ...
    let time_since_damage = Instant::now().elapsed().as_secs_f64() - damage_source.last_damage_seconds;
    if time_since_damage > DAMAGE_REWARD_EXPIRE_TIME.as_secs_f64() {
