...
use rose_game_common::{
    components::{BasicStatType, ClanLevel, ClanPoints, DroppedItem, ExperiencePoints, SkillSlot, MAX_STAMINA},
    data::Damage,
};
...
use crate::game::{
    ...
    components::{
        ...
        ClientEntityType, Command, Cooldowns, DamageSources, Dead, EquipmentItemDatabase,
        GameClient, GhostMode, GodMode, HealthPoints, Inventory, Level, ManaPoints, Money,
    ...
    events::{
        ChatCommandEvent, ChatMessageEvent, ClanEvent, DamageEvent, ReviveEvent, RevivePosition,
        RewardItemEvent, RewardXpEvent, SaveEvent,
    },
...
lazy_static! {
    pub static ref CHAT_COMMANDS: clap::Command<'static> = {
        clap::Command::new("Chat Commands")
            ...
            .subcommand(clap::Command::new("mspeed").arg(Arg::new("speed").required(true)))
            .subcommand(
            ...
            .subcommand(clap::Command::new("ghost"))
            .subcommand(clap::Command::new("heal"))
            .subcommand(clap::Command::new("tp").arg(Arg::new("player_name").required(true)))
            .subcommand(clap::Command::new("announce").arg(Arg::new("message").required(true)))
            .subcommand(clap::Command::new("players"))
            .subcommand(clap::Command::new("serverinfo"))
            .subcommand(clap::Command::new("info"))
            .subcommand(clap::Command::new("revive"))
            .subcommand(clap::Command::new("money").arg(Arg::new("amount").required(true)))
            .subcommand(clap::Command::new("save"))
            .subcommand(clap::Command::new("who"))
            .subcommand(clap::Command::new("zonelist"))
    };
}
...
fn handle_chat_command(
        ...
        .ok_or(ChatCommandError::InvalidCommand)?
    {
        ...
        ("god", _) => {
            if chat_command_user.god_mode.is_some() {
                ...
                chat_command_user
                    ...
                    .send(ServerMessage::Whisper {
                        from: String::from("SERVER"),
                        text: String::from("God mode DISABLED"),
                    })
                    .ok();
            } else {
                // Add god mode
                ...
                chat_command_user
                    ...
                    .send(ServerMessage::Whisper {
                        from: String::from("SERVER"),
                        text: String::from("God mode ENABLED - You are now invincible!"),
                    })
                    .ok();
            }
        }
        // /ghost - Toggle ghost mode (no collision, invisible to monsters)
        ("ghost", _) => {
            if chat_command_user.ghost_mode.is_some() {
                // Remove ghost mode
                chat_command_params
                    .commands
                    .entity(chat_command_user.entity)
                ...
                chat_command_user
                    ...
                    .send(ServerMessage::Whisper {
                        from: String::from("SERVER"),
                        text: String::from("Ghost mode DISABLED"),
                    })
                    .ok();
            } else {
                // Add ghost mode
                ...
                chat_command_user
                    ...
                    .send(ServerMessage::Whisper {
                        from: String::from("SERVER"),
                        text: String::from("Ghost mode ENABLED - No collision, invisible to monsters!"),
                    })
                    .ok();
            }
        }
        // /heal - Fully restore HP/MP/Stamina
        ("heal", _) => {
            let max_hp = chat_command_user.ability_values.get_max_health();
            let max_mp = chat_command_user.ability_values.get_max_mana();

            chat_command_user.health_points.hp = max_hp;
            chat_command_user.mana_points.mp = max_mp;
            chat_command_user.stamina.stamina = MAX_STAMINA;
            ...
            chat_command_user
                ...
                .server_message_tx
                .send(ServerMessage::UpdateAbilityValueSet {
                    ability_type: AbilityType::Stamina,
                    value: MAX_STAMINA as i32,
                })
                .ok();
            ...
            chat_command_user
                ...
                .send(ServerMessage::Whisper {
                    from: String::from("SERVER"),
                    text: format!(
                        "Healed! HP: {}, MP: {}, Stamina: {}",
                        max_hp, max_mp, MAX_STAMINA
                    ),
                })
                .ok();
        }
        // /tp <player_name> - Teleport to a specific player
        ("tp", arg_matches) => {
            let target_name = arg_matches.value_of("player_name").unwrap().to_lowercase();
            ...
            if let Some(client_entity_zone) = chat_command_params
                ...
                .get_zone(chat_command_user.position.zone_id)
            {
                for (entity, client_entity_type) in client_entity_zone.iter_entities() {
                    if client_entity_type == ClientEntityType::Character {
                        // We need to query the character info - this is a simplified approach
                        // In practice, we'd need a separate query to get CharacterInfo by entity
                        // For now, let's search through all zones
                    }
                }
            }

            // Search all zones for the player
            for zone_id in chat_command_params.game_data.zones.iter_zone_ids() {
                if let Some(client_entity_zone) =
                    chat_command_params.client_entity_list.get_zone(zone_id)
                {
                    for (entity, client_entity_type) in client_entity_zone.iter_entities() {
                        if client_entity_type == ClientEntityType::Character {
                            // For simplicity, we'll teleport to the entity position
                            // A full implementation would verify the player name
                            let _ = entity; // Placeholder for actual name lookup
                        }
                    }
                }
            }
            ...
            if let Some(target_pos) = target_position {
                ...
                chat_command_user
                    ...
                    .send(ServerMessage::Whisper {
                        from: String::from("SERVER"),
                        text: format!("Teleported to player: {}", target_name),
                    })
                    .ok();
            } else {
                return Err(ChatCommandError::WithMessage(format!(
        ...
        ("announce", arg_matches) => {
            let message = arg_matches.value_of("message").unwrap();

            chat_command_params.server_messages.send_global_message(
                ServerMessage::AnnounceChat {
                    name: Some(chat_command_user.character_info.name.clone()),
            ...
            chat_command_user
                ...
                .send(ServerMessage::Whisper {
                    from: String::from("SERVER"),
                    text: String::from("Announcement sent!"),
                })
                .ok();
        }
        // /players - List all online players
        ("players", _) => {
            let mut player_list = String::from("Online Players:\n");
            let mut count = 0;
            ...
            for zone_id in chat_command_params.game_data.zones.iter_zone_ids() {
                if let Some(client_entity_zone) =
                    chat_command_params.client_entity_list.get_zone(zone_id)
                {
                    for (_entity, client_entity_type) in client_entity_zone.iter_entities() {
                        if client_entity_type == ClientEntityType::Character {
                            count += 1;
                            // Note: Full implementation would query CharacterInfo for each entity
                        }
                    }
                }
            }

            player_list.push_str(&format!("\nTotal players online: {}", count));

            send_multiline_whisper(chat_command_user.game_client, &player_list);
        }
        ...
        ("serverinfo", _) => {
            ...
            for zone_id in chat_command_params.game_data.zones.iter_zone_ids() {
                ...
                {
                    for (_entity, client_entity_type) in client_entity_zone.iter_entities() {
                        match client_entity_type {
                            ClientEntityType::Character => player_count += 1,
            ...
            let info = format!(
                ...
                monster_count,
                npc_count,
                chat_command_params.world_rates.xp_rate,
                chat_command_params.world_rates.drop_rate,
                chat_command_params.world_rates.drop_money_rate
            );

            send_multiline_whisper(chat_command_user.game_client, &info);
        }
        // /info - Debug entity info under cursor (simplified - shows own entity info)
        ("info", _) => {
            let info = format!(
                "Entity Info:\n\
                 Entity ID: {:?}\n\
                 ...
                 Stamina: {}",
                chat_command_user.entity,
                chat_command_user.client_entity.id,
                chat_command_user.character_info.name,
                chat_command_user.level.level,
                chat_command_user.position.zone_id.get(),
                chat_command_user.position.position.x,
                chat_command_user.position.position.y,
                chat_command_user.position.position.z,
                chat_command_user.health_points.hp,
                chat_command_user.ability_values.get_max_health(),
                chat_command_user.mana_points.mp,
                chat_command_user.ability_values.get_max_mana(),
                chat_command_user.stamina.stamina
            );
        ...
        ("revive", _) => {
            ...
            if chat_command_user.dead.is_some() {
                ...
                // Restore some HP
                let revive_hp = ((chat_command_user.ability_values.get_max_health() as f32 * 0.3) as i32).max(1);
                chat_command_user.health_points.hp = revive_hp;
                ...
                chat_command_user
                    ...
                    .send(ServerMessage::Whisper {
                        from: String::from("SERVER"),
                        text: String::from("You have been revived!"),
                    })
                    .ok();
            } else {
                return Err(ChatCommandError::WithMessage(
                    "You are not dead!".to_string(),
                ));
            }
        }
        // /money <amount> - Set money directly
        ("money", arg_matches) => {
            let amount = arg_matches.value_of("amount").unwrap().parse::<i64>()?;
            ...
            chat_command_user.inventory.money = Money(amount);
            ...
            chat_command_user
                ...
                .send(ServerMessage::Whisper {
                    from: String::from("SERVER"),
                    text: format!("Money set to: {}", amount),
                })
                .ok();
        }
        ...
        ("save", _) => {
            ...
            chat_command_user
                ...
                .send(ServerMessage::Whisper {
                    from: String::from("SERVER"),
                    text: String::from("Character save triggered!"),
                })
                .ok();
        }
        // /who - Show nearby players in the same zone
        ("who", _) => {
            let mut nearby_players = String::from("Nearby Players:\n");
            let mut count = 0;
                ...
                .get_zone(chat_command_user.position.zone_id)
            {
                for (_entity, client_entity_type) in client_entity_zone.iter_entities() {
                    if client_entity_type == ClientEntityType::Character {
                        count += 1;
                        // Full implementation would include name, level, etc.
                    }
                }
            }

            nearby_players.push_str(&format!(
                "Zone: {}\nPlayers in zone: {}",
                chat_command_user.position.zone_id.get(),
                count
            ));

            send_multiline_whisper(chat_command_user.game_client, &nearby_players);
        }
        // /zonelist - List all available zones with IDs
        ("zonelist", _) => {
            let mut zone_list = String::from("Available Zones:\n");

            for zone_id in chat_command_params.game_data.zones.iter_zone_ids() {
                if let Some(zone_data) = chat_command_params.game_data.zones.get_zone(zone_id) {
                    zone_list.push_str(&format!(
                        "ID: {} - {:?}\n",
                        zone_id.get(),
                        zone_data.name
                    ));
            ...
            zone_list.push_str("\nUse /mm <zone_id> to teleport to a zone.");

            send_multiline_whisper(chat_command_user.game_client, &zone_list);
        }
